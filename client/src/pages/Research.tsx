...
import { ArrowRight, Terminal, Code2, Shield, Cpu, Database, Globe, Layers, Zap } from "lucide-react";
...
// Default images for first two research areas
const defaultImages = [RESEARCH_SYSTEMS, RESEARCH_COMPILER];

// Default research areas (used when DB is empty)
const defaultResearchDetails = [
  {
    title: "운영체제", titleEn: "Operating Systems", icon: "Terminal",
    image: RESEARCH_SYSTEMS,
    description: "운영체제는 컴퓨터 시스템의 핵심 소프트웨어로, 하드웨어 자원을 효율적으로 관리하고 사용자에게 편리한 인터페이스를 제공합니다. SSLAB에서는 리눅스 커널 최적화, 실시간 스케줄링 알고리즘, 가상화 기술, 컨테이너 기반 격리 기법 등을 연구합니다.",
    topics: "리눅스 커널 성능 최적화,실시간 태스크 스케줄링,컨테이너 및 가상화 기술,메모리 관리 기법,파일 시스템 설계",
  },
  {
    title: "컴파일러", titleEn: "Compiler Design", icon: "Code2",
    image: RESEARCH_COMPILER,
    description: "컴파일러는 고급 프로그래밍 언어를 기계어로 변환하는 핵심 시스템 소프트웨어입니다. SSLAB에서는 프로그램 분석, 코드 최적화, 정적 분석 기법, 중간 표현(IR) 설계 등 컴파일러 기술 전반을 연구합니다.",
    topics: "정적 프로그램 분석,코드 최적화 기법,LLVM 기반 컴파일러 개발,도메인 특화 언어(DSL) 설계,Just-In-Time 컴파일",
  },
  {
    title: "시스템 보안", titleEn: "System Security", icon: "Shield",
    description: "시스템 보안은 운영체제와 시스템 소프트웨어 수준에서 발생하는 보안 위협에 대응하는 연구 분야입니다. 메모리 안전성, 커널 취약점 분석, 접근 제어 메커니즘, 하드웨어 기반 보안 기법 등을 연구합니다.",
    topics: "메모리 안전성 보장 기법,커널 취약점 분석 및 방어,하드웨어 기반 보안 (TEE SGX),퍼징(Fuzzing) 기반 취약점 탐지,접근 제어 및 격리 기법",
  },
  {
    title: "임베디드 시스템", titleEn: "Embedded Systems", icon: "Cpu",
    description: "임베디드 시스템은 특정 기능을 수행하도록 설계된 컴퓨터 시스템으로, IoT 디바이스부터 자동차, 의료기기까지 다양한 분야에 적용됩니다. SSLAB에서는 실시간 운영체제, 저전력 컴퓨팅, 엣지 컴퓨팅 등을 연구합니다.",
    topics: "실시간 운영체제 (RTOS),IoT 디바이스 소프트웨어,저전력 컴퓨팅 최적화,엣지 컴퓨팅 플랫폼,센서 네트워크 시스템",
  },
];
...
export default function Research() {
  ...
  const researchDetails = useMemo(() => {
    if (researchQuery.data && researchQuery.data.length > 0) {
      return researchQuery.data.map((area, idx) => ({
        number: String(idx + 1).padStart(2, "0"),
        title: area.title,
        titleEn: area.titleEn,
        icon: area.icon,
        image: area.imageUrl || (idx < defaultImages.length ? defaultImages[idx] : null),
        description: area.description,
        topics: area.topics.split(",").map((t: string) => t.trim()),
      }));
    }
    return defaultResearchDetails.map((area, idx) => ({
      number: String(idx + 1).padStart(2, "0"),
      ...area,
      image: area.image || null,
      topics: area.topics.split(",").map(t => t.trim()),
    }));
  }, [researchQuery.data]);
                  ...
                  <motion.div variants={fadeUp} custom={0} className="relative mb-10">
                    ...
                    <div className="relative z-10 pt-14">
                      <div className="flex items-center gap-3 mb-3">
                        <div className="accent-line" />
                        <span className="font-mono text-signal-red text-xs tracking-[0.2em] uppercase">{area.titleEn}</span>
                      </div>
                      <h2 className="font-heading font-bold text-3xl lg:text-4xl text-navy">{area.title}</h2>
                    </div>
                  </motion.div>
                ...
                >
                  {area.image ? (
                    <div className="relative sticky top-28">
                      ...
                      <img src={area.image} alt={area.title} className="w-full aspect-[4/3] object-cover relative z-10" />
                    ...
                    <div className="relative aspect-[4/3] bg-navy flex items-center justify-center sticky top-28 overflow-hidden">
                      ...
                      <div className="relative z-10 flex flex-col items-center gap-4">
                        <div className="w-20 h-20 bg-signal-red/10 flex items-center justify-center">
                          <IconComponent size={36} className="text-signal-red/60" />
                        </div>
                        <span className="font-heading font-bold text-[6rem] text-white/[0.06]">{area.number}</span>
                      </div>
